...
volatile int synchro; //volatile: on demande au compilateur de toujours accéder à la vraie valeur de la variable synchro

void *fn_serveur_tcp(void *arg) //une fonction thread
{
...
void sendMessageToServer(char *ipAddress, int portno, char *mess) //envoyer des messages au serveur principal
{
    int sockfd, n;
...
int main(int argc, char ** argv)
{
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL); 
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				//printf("mx=%d my=%d\n",mx,my);
				if ((mx<200) && (my<50) && (connectEnabled==1)) //bouton connect est affiché(connectEnabled ==1)
				{
					sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName);
					//quand on clique sur le bouton, un message sera envoyé au serveur globale
    ...
    if (synchro==1) //le serveur a recu un message, il faut le consulter
    {
        printf("consomme |%s|\n",gbuffer);
		switch (gbuffer[0])
		{
			...
			case 'V':{
				int row, col, nb;
				sscanf(gbuffer, "V %d %d %d", &row, &col, &nb);
				if (tableCartes[row][col] == -1 || tableCartes[row][col] == 100)
					tableCartes[row][col] = nb;
				break;
			}
